...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
      if dh_prev is None:
        ...
        dht = dh[i] + dh_prev
      dxi, dh_prev, dWxi, dWhi, dbi = rnn_step_backward(dht, cache[i])
      if dx is None:
        _, D = dxi.shape
        dx = torch.zeros((N, T, D), dtype=dh.dtype, device=dh.device)
        dx = torch.permute(dx, (1, 0, 2))
        dWx, dWh, db = torch.zeros_like(dWxi), torch.zeros_like(dWhi), torch.zeros_like(dbi)
      dWx += dWxi; dWh += dWhi; db += dbi
      dx[i] = dxi
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N, T, V = x.shape
    loss = torch.nn.functional.cross_entropy(x.reshape(N*T, V), y.reshape(N*T), ignore_index=ignore_index, reduction='sum')
    loss /= N
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.feature_extraction = ImageEncoder(pretrained=image_encoder_pretrained, verbose=False)
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        features = self.feature_extraction.forward(images)
        x = self.word_embedding.forward(captions_in)
        if self.cell_type == 'rnn' or self.cell_type == 'lstm':
          h0 = self.feature_proj.forward(features)
          h = self.model.forward(x, h0)
        elif self.cell_type == 'attn':
          A = self.feature_proj.forward(features.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
          h = self.model.forward(x, A)
        scores = self.output_proj.forward(h)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == 'rnn':
          h_prev = self.feature_proj.forward(features)
          ...
          c_prev = A.mean(dim=(2, 3))
        words = self._start * torch.ones(N).long()

        for i in range(max_length):
          x = self.word_embedding.forward(words)
          if self.cell_type == 'rnn':
            h_prev = self.model.step_forward(x, h_prev)
          elif self.cell_type == 'lstm':
            h_prev, c_prev = self.model.step_forward(x, h_prev, c_prev)
          elif self.cell_type == 'attn':
            attn, attn_weights_all[:, i, :, :] = dot_product_attention(h_prev, A)
            h_prev, c_prev = self.model.step_forward(x, h_prev, c_prev, attn)
          scores = self.output_proj.forward(h_prev)
          words = torch.argmax(scores, dim=1)
          captions[:, i] = words
        
        # Append <start> to the front
        starts = torch.full((N, 1), self._start, device=captions.device).long()
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        I_ = torch.sigmoid(A[:, :H])
        F_ = torch.sigmoid(A[:, H:2*H])
        O_ = torch.sigmoid(A[:, 2*H:3*H])
        G_ = torch.tanh(A[:, 3*H:])

        next_c = F_ * prev_c + I_ * G_
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        h = h0
        c = c0
        hn = torch.zeros((N, T, H)).to(x)

        for i in range(T):
          h, c = self.step_forward(x[:, i, :], h, c)
          hn[:, i, :] = h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        next_h, next_c = None, None
        # Replace "pass" statement with your code
        a = torch.mm(prev_h,self.Wh) + torch.mm(x,self.Wx) + torch.mm(attn, self.Wattn) + self.b
        H = a.shape[1] // 4
