...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i, j] = (x_train_flatten[i] - x_test_flatten[j]).pow(2).sum().sqrt().item()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten = x_test.reshape(num_test, -1)
    for i in range(num_train):
      dists[i] = (x_train_flatten[i] - x_test_flatten).pow(2).sum(dim=1).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # (a - b)^2 = a^2 + b^2 - 2ab
    x_train_flatten_squared_sum_by_row = x_train_flatten.pow(2).sum(dim=1).unsqueeze(1) # a^2
    x_test_flatten_squared_sum_by_row = x_test_flatten.pow(2).sum(dim=1).unsqueeze(0) # b^2

    dists = (x_train_flatten_squared_sum_by_row + x_test_flatten_squared_sum_by_row \
            - 2 * (x_train_flatten @ x_test_flatten.t())).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values, indices = torch.topk(dists, dim=0, k=k, sorted=True, largest=False)
    for j in range(num_test):
      uniqued, counts = torch.unique(y_train[indices[:, j]], sorted=True, return_counts=True)
      y_pred[j] = uniqued[torch.argmax(counts)]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
