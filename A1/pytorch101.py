...
from torch import Tensor, col_indices_copy
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    first = (start + 10 - 1) // 10 * 10
    if first > stop:
      x = torch.tensor([], dtype=torch.float64)
    else:
      x = torch.arange(first, stop+1, 10).to(torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mu = x.sum(dim=0, keepdim=True) / x.shape[1]
    sigma = (x - mu) ** 2
    sigma = sigma.sum(dim=0) / (x.shape[0] - 1)
