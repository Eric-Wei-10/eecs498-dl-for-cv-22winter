...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.decoder = nn.Sequential(
          ...
          nn.Sigmoid(),
          nn.Unflatten(1, (1, self.input_size))
        )
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
    ...
    def forward(self, x):
        ...
        logvar = self.logvar_layer(feature_map)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        x_cat = torch.cat((x.reshape(x.shape[0], -1), c), dim=-1)
        feature_map = self.encoder(x_cat)
        mu = self.mu_layer(feature_map)
        logvar = self.logvar_layer(feature_map)
        z = reparametrize(mu, logvar)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    DKL = -0.5 * torch.sum(1 + logvar - torch.pow(mu, 2) - torch.exp(logvar))
    loss = DKL + reconstruction_loss
    loss /= x.shape[0]
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
