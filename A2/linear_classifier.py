...
def svm_loss_vectorized(
    ...
    scores = X @ W
    correct_class_scores = scores[torch.arange(num_train), y].unsqueeze(dim=-1)
    loss_matrix = (scores - correct_class_scores + 1)
    loss_matrix[torch.arange(num_train), y] = 0
    ...
    #############################################################################
    # Replace "pass" statement with your code
    mask = (loss_matrix > 0).double()

    # 统计每个样本的margin违规次数 (N,)
    margin_counts = mask.sum(dim=1)

    mask[torch.arange(num_train), y] = -margin_counts  # 正确类别位置设为负计数
...
def softmax_loss_vectorized(
    ...
    loss = (-torch.log(scores[torch.arange(num_train), y])).mean().item()
    loss += reg * torch.sum(W * W)
