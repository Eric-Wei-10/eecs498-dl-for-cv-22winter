...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden, cache_1 = Linear.forward(X, W1, b1)
        ...
        dhidden, dW2, db2 = Linear.backward(dscores, cache_2)
        _, dW1, db1 = Linear.backward(dhidden, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          self.params['b%s' % (i+1)] = torch.zeros(self.dims[i+1], dtype=dtype, device=device)
          self.params['W%s' % (i+1)] = torch.normal(0, std=weight_scale, size=(self.dims[i], self.dims[i+1]), dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          if i == self.num_layers-1:
            out, linear_cache = Linear.forward(out, self.params['W%s'%(i+1)], self.params['b%s'%(i+1)])
          else:
            out, linear_cache = Linear_ReLU.forward(out, self.params['W%s'%(i+1)], self.params['b%s'%(i+1)])
        ...
        dout = dscores
        for i, linear_cache in reversed(list(enumerate(linear_caches))):
          Wi = self.params['W%s'%(i+1)]
          loss += 0.5 * self.reg * torch.sum(Wi*Wi)
          ...
          if i == self.num_layers-1:
            ...
            dout, dW, db = Linear_ReLU.backward(dout, linear_cache)
          grads['W%s'%(i+1)] = dW + self.reg * Wi
          grads['b%s'%(i+1)] = db
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=1000, reg=0.002, dtype=dtype, device=device)
    #############################################################
    ...
    solver = Solver(model, data_dict, 
                    device=device,
                    num_train_samples=data_dict['X_train'].shape[0],
                    batch_size=100,
                    num_epochs=20,
                    lr_decay=0.99,
                    optim_config={
...
def get_three_layer_network_params():
    ...
    learning_rate = 0.22  # Experiment with this!
    # Replace "pass" statement with your code
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - learning_rate * (dw / (grad_squared.sqrt() + epsilon))
...
def adam(w, dw, config=None):
    ...
    grad_m = config.get('m')
    grad_squared_m = config.get('v')
    t = config.get('t')
    t += 1

    grad_m = beta1 * grad_m + (1 - beta1) * dw
    grad_squared_m = beta2 * grad_squared_m + (1 - beta2) * dw * dw

    grad_m_unbias = grad_m / (1 - beta1 ** t)
    grad_squared_m_unbias = grad_squared_m / (1 - beta2 ** t)

    next_w = w - learning_rate * grad_m_unbias / (grad_squared_m_unbias.sqrt() + epsilon)
    config['m'] = grad_m
    config['v'] = grad_squared_m
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(x.shape, device=x.device) > p).to(x.dtype)
            mask = mask / p
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
