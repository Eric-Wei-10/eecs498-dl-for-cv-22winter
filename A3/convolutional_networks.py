...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        dilation = conv_param.get('dilation', 0)
        x_pad = F.pad(x, (pad, pad, pad, pad))

        N = x.shape[0]
        O = w.shape[0]
        H_in_pad = x_pad.shape[2]
        W_in_pad = x_pad.shape[3]
        H_filter = w.shape[2]
        W_filter = w.shape[3]
        H_out = 1 + (H_in_pad - H_filter) // stride
        W_out = 1 + (W_in_pad - W_filter) // stride
        out = torch.zeros((N, O, H_out, W_out), dtype=x.dtype, device=x.device)

        for n in range(N):
          for o in range(O):
            for i in range(0, H_in_pad - H_filter + stride, stride):
              for j in range(0, W_in_pad - W_filter + stride, stride):
                out[n, o, i//stride, j//stride] = b[o] + torch.sum(x_pad[n, :, i:i+H_filter:dilation+1, j:j+W_filter:dilation+1] * w[o])
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
          ...
          for c in range(C):
            for i in range(H_filter):
              for j in range(W_filter):
                dw[o, c, i, j] = torch.sum(dout[:, o] * x_pad[:, c, i:i+H_out*stride:stride, j:j+W_out*stride:stride])

        for n in range(N):
          for o in range(O):
            for i in range(H_out):
              for j in range(W_out):
                dx_pad[n, :, i*stride:i*stride+H_filter, j*stride:j*stride+W_filter] += dout[n, o, i, j] * w[o]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        ####################################################################
        # Replace "pass" statement with your code
        H_pool = pool_param.get('pool_height', 2)
        W_pool = pool_param.get('pool_width', 2)
        stride = pool_param.get('stride', H_pool)
        N, C, H_in, W_in = x.shape
        ...
        for i in range(H_out):
          for j in range(W_out):
            out[:, :, i, j] = torch.amax(x[:, :, i*stride:i*stride+H_pool, j*stride:j*stride+W_pool], (2, 3))
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for c in range(C):
            for i in range(H_out):
              for j in range(W_out):
                x_partial = x[n, c, i*stride:i*stride+H_pool, j*stride:j*stride+W_pool]
                ii, jj = (x_partial == x_partial.max()).nonzero()[0]
                dx[n, c, i*stride+ii, j*stride+jj] += 1.0 * dout[n, c, i, j]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        H_1, W_1 = H // 2, W // 2
        self.params['W2'] = weight_scale * torch.randn(num_filters*H_1*W_1, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        h2, linear_relu_cache = Linear_ReLU.forward(h1, W2, b2)
        scores, linear_cache = Linear.forward(h2, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dh2, dW3, db3 = Linear.backward(dscores, linear_cache)
        dh1, dW2, db2 = Linear_ReLU.backward(dh2, linear_relu_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        # Replace "pass" statement with your code
        C_i, H_i, W_i = input_dims
        for i, num_filter in enumerate(num_filters):
          if weight_scale == 'kaiming':
            self.params[f'W{i+1}'] = kaiming_initializer(C_i, num_filter, K=3, relu=True, device=device, dtype=dtype)
          else:
            self.params[f'W{i+1}'] = torch.randn((num_filter, C_i, 3, 3), dtype=dtype, device=device) * weight_scale
          ...
          if i in max_pools:
            ...
            W_i //= 2
          if batchnorm:
            self.params[f'gamma{i+1}'] = torch.ones(C_i, dtype=dtype, device=device)
            self.params[f'beta{i+1}'] = torch.zeros(C_i, dtype=dtype, device=device)
        if weight_scale == 'kaiming':
          self.params[f'W{self.num_layers}'] = kaiming_initializer(C_i*H_i*W_i, num_classes, relu=True, device=device, dtype=dtype)
        else:
          self.params[f'W{self.num_layers}'] = torch.randn((C_i*H_i*W_i, num_classes), dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        if not self.batchnorm:
          for i, cache in reversed(list(enumerate(caches))):
            if i != self.num_layers-1:
              if i in self.max_pools:
                dout, dW_i, db_i = Conv_ReLU_Pool.backward(dout, cache)
              else:
                dout, dW_i, db_i = Conv_ReLU.backward(dout, cache)
            else:
              dout, dW_i, db_i = Linear.backward(dout, cache)
            grads[f'W{i+1}'] = dW_i + 2 * self.reg * self.params[f'W{i+1}']
            grads[f'b{i+1}'] = db_i
        else:
          for i, cache in reversed(list(enumerate(caches))):
            if i != self.num_layers-1:
              if i in self.max_pools:
                dout, dW_i, db_i, dgamma_i, dbeta_i = Conv_BatchNorm_ReLU_Pool.backward(dout, cache)
              else:
                dout, dW_i, db_i, dgamma_i, dbeta_i = Conv_BatchNorm_ReLU.backward(dout, cache)
              grads[f'gamma{i+1}'] = dgamma_i
              grads[f'beta{i+1}'] = dbeta_i
            else:
              dout, dW_i, db_i = Linear.backward(dout, cache)
            grads[f'W{i+1}'] = dW_i + 2 * self.reg * self.params[f'W{i+1}']
            grads[f'b{i+1}'] = db_i
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    model = DeepConvNet(input_dims=data_dict['X_train'].shape[1:], num_filters=[8, 16, 32, 32], max_pools=[0, 2], weight_scale='kaiming', reg=3e-3, dtype=torch.double, device='cuda', batchnorm=False)

    optim_config = {
        'learning_rate': 1e-1
    }

    solver = Solver(model, data_dict, optim_config=optim_config, lr_decay=0.97, batch_size=128, num_epochs=40, print_every=50, print_acc_every=1, device='cuda')
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
    return solver
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn((Din, Dout), dtype=dtype, device=device) * math.sqrt(gain / Din)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn((Dout, Din, K, K), dtype=dtype, device=device) * math.sqrt(gain / (Din * K * K))
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x, batch_mean, batch_var, x_hat, eps, gamma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            cache = (x, running_mean, running_var, x_hat, eps, gamma)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        # dx_hat = dout * gamma
        # dvar = (dx_hat * (-0.5 * x_hat / (var + eps))).sum(dim=0)
        # dmean = -2 * dvar * torch.mean(x - mean, dim=0) - (dx_hat / (var + eps).sqrt()).sum(dim=0)
        # dx = 2 * dvar * (x - mean) / N + dx_hat / (var + eps).sqrt() + dmean / N
    ...
    def backward_alt(dout, cache):
        ...
        dgamma = (dout * x_hat).sum(dim=0)
        dbeta = dout.sum(dim=0)
        # NOTE: x_i contributes to every x_j_hat. The result is dX = (dY - mean(dY) - X_hat * mean(dY * X_hat)) / σ
        dx = (gamma / (var + eps).sqrt()) * (dout - x_hat * (dout * x_hat).mean(dim=0) - dout.mean(dim=0))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        out, cache = BatchNorm.forward(x.permute((0, 2, 3, 1)).reshape((-1, C)), gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dx, dgamma, dbeta = BatchNorm.backward_alt(dout.permute((0, 2, 3, 1)).reshape((-1, C)), cache)
